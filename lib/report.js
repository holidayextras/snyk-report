...
Report._formatVulns = function(groupedData) {
  return Report._risks.reduce(function(previous, risk) {
    ...
    return groupedData[risk.name].reduce(function(innerPrevious, vuln) {
      innerPrevious += colours[risk.colour](risk.name.toUpperCase() + ':\t' + vuln.name + ' - ' + vuln.title + '\n');
      innerPrevious += colours[risk.colour]('\tIn: ' + vuln.path.join(' -> ') + '\n');
      innerPrevious += colours[risk.colour]('\tAge: ' + vuln.age + ' days old\n');
      innerPrevious += colours[risk.colour]('\tMore info: https://app.snyk.io/vuln/' + vuln.id + '\n\n');
